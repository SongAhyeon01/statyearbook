# Step 5: stat_id_name 에서 statblTag='T' 인 항목에 대해
#         statDataList.do 로 실제 값을 가져와
#         stat_yearbook.values.<year> 에 iCol_* 값으로 저장한다.

import httpx
from motor.motor_asyncio import AsyncIOMotorDatabase

from app.config.settings import settings
from app.config.logging import get_logger
from app.config.exceptions import MoisApiException

logger = get_logger("batch.step5_values")

STAT_DATA_URL = f"{settings.MOIS_API_BASE_URL}/mois/visual/statDataList.do"
SRC_COLLECTION = "stat_id_name"
DST_COLLECTION = "stat_yearbook"


async def fetch_stat_data(
    statbl_id: str, start_year: int, end_year: int
) -> dict:
    """statblId 로 통계 데이터를 조회한다."""
    async with httpx.AsyncClient() as client:
        response = await client.post(
            STAT_DATA_URL,
            params={"ibpage": 1},
            data={
                "statblId": statbl_id,
                "wrttimeStartYear": start_year,
                "wrttimeEndYear": end_year,
            },
            timeout=30,
        )

    if response.status_code != 200:
        raise MoisApiException(
            f"statDataList API 호출 실패: statblId={statbl_id}, status={response.status_code}"
        )

    return response.json()


def _progress_bar(current: int, total: int, width: int = 30) -> str:
    ratio = current / total
    filled = int(width * ratio)
    bar = "█" * filled + "░" * (width - filled)
    return f"[{bar}] {current}/{total} ({ratio:.0%})"


async def load_all(
    db: AsyncIOMotorDatabase, start_year: int, end_year: int
) -> dict:
    """statblTag='T' 대상에 대해 값을 가져와 stat_yearbook.values 에 저장."""
    src = db[SRC_COLLECTION]
    dst = db[DST_COLLECTION]

    cursor = src.find({"statblTag": "T"}, {"_id": 0})
    targets = await cursor.to_list(length=None)
    logger.info(f"{SRC_COLLECTION} 에서 statblTag='T' 항목 {len(targets)}건 조회")

    if not targets:
        logger.warning("statblTag='T' 인 항목이 없습니다")
        return {"success": 0, "fail": 0, "skip": 0}

    success_count = 0
    fail_count = 0
    skip_count = 0
    total = len(targets)
    log_interval = max(1, total // 10)

    for idx, item in enumerate(targets, 1):
        statbl_id = item["statblId"]
        statbl_nm = item.get("statblNm", "")

        try:
            result = await fetch_stat_data(statbl_id, start_year, end_year)

            result_code = result.get("RESULT", {}).get("CODE")
            if result_code != 0:
                logger.warning(
                    f"API 응답 에러: {statbl_nm}({statbl_id}) CODE={result_code}"
                )
                fail_count += 1
                continue

            data = result.get("DATA", [])
            if not data:
                skip_count += 1
                continue

            year_values = {}
            for row in data:
                dyear = row.get("dYear")
                if not dyear:
                    continue
                year_values[dyear] = {
                    k: v for k, v in row.items() if k.startswith("iCol_")
                }

            if year_values:
                set_fields = {f"values.{y}": v for y, v in year_values.items()}
                await dst.update_one(
                    {"statblId": statbl_id},
                    {
                        "$set": set_fields,
                        "$setOnInsert": {
                            "statblNm": statbl_nm,
                            "metadata": {},
                        },
                    },
                    upsert=True,
                )

            success_count += 1

        except MoisApiException as e:
            fail_count += 1
            logger.error(f"API 호출 실패: {statbl_nm}({statbl_id}) - {e}")
        except Exception as e:
            fail_count += 1
            logger.error(
                f"예상치 못한 오류: {statbl_nm}({statbl_id}) - {type(e).__name__}: {e}"
            )

        if idx % log_interval == 0 or idx == total:
            logger.info(
                f"진행: {_progress_bar(idx, total)} "
                f"(성공={success_count} 실패={fail_count} 건너뜀={skip_count})"
            )

    summary = {"success": success_count, "fail": fail_count, "skip": skip_count}
    logger.info(f"{DST_COLLECTION}.values 저장 결과: {summary}")
    return summary


async def run(
    db: AsyncIOMotorDatabase, start_year: int, end_year: int
) -> dict:
    """Step 5 실행: statDataList.do → stat_yearbook.values."""
    logger.info(
        f"===== Step 5: stat_yearbook 값 수집 시작 (years={start_year}~{end_year}) ====="
    )
    summary = await load_all(db, start_year, end_year)
    logger.info(f"===== Step 5 완료: {summary} =====")
    return summary
